...
public class OaiPublicationDetailsBeanFactory {
	...
	public OaiPublicationDetailsBeanFactory(String organizationId, Date startDate, Date endDate) {
		super();
	...
	public List<OaiPublicationDetailsBean> getOaiPublicationDetailsBeans(){			
		...
		while (it.hasNext()){
			...
			OaiPublicationDetailsBeanFactory oaifactory = new OaiPublicationDetailsBeanFactory(organizationId, startDate, endDate);
			List<OaiPublicationDetailsBean> pubs = new ArrayList<OaiPublicationDetailsBean>();
			pubs = oaifactory.getOaiPublicationDetailsBeans(organizationId,organizationName);
			all.addAll(pubs);
		}
	...
	public List<OaiPublicationDetailsBean> getOaiPublicationDetailsBeans(String orgid,String organizationName){				
		...
		public List<OaiPublicationDetailsBean> getOaiPublicationDetailsBeans(JSONObject result,String organizationName){
			...
			while (it.hasNext()){
				...
				Date created = null;
				try {
					created = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").parse(dateCreated);
				...
				if (jsonObject.get("insertedRecords")!=null){
					 insertedRecords =  (Integer) jsonObject.get("insertedRecords") / 2	;
				}
				//else insertedRecords = 0;
				
				if (jsonObject.get("conflictedRecords")!=null){ 
					conflictedRecords =  (Integer) jsonObject.get("conflictedRecords") / 2 ;
				}
				//else conflictedRecords = 0;
				
				
				OaiPublicationDetailsBean oaipublicationbean = new OaiPublicationDetailsBean(projectName, organizationName,organizationId, datestamp, created, insertedRecords, conflictedRecords);
		...
		public Integer gettotalValidItemsPublished(){
			...
			List<OaiPublicationDetailsBean> tlist = this.getOaiPublicationDetailsBeans();
			
			Iterator<OaiPublicationDetailsBean> it = tlist.iterator();
			
			while (it.hasNext()){
				OaiPublicationDetailsBean oaibean = (OaiPublicationDetailsBean) it.next();
				totalValidPublished += oaibean.getInsertedRecords();
			}
			
		/*	if (tlist.size()>0){
			OaiPublicationDetailsBean oaibean = tlist.get(tlist.size() -1);
			totalValidPublished = oaibean.getInsertedRecords();
			}
			*/
