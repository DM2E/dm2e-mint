...
public class DataUploadDetailsBeanFactory {
	...
	private DerivedTransformationDetailsBeanFactory derivedBeanFactory;
	...
	public DataUploadDetailsBeanFactory(String organizationId, Date startDate, Date endDate) {
		super();
	 ...
	 * commented this and added 2 methods below to allow grouping per organization needed for jasper result
...
//		JSONObject json =  jsFetcher.getJson();
		...
		while (it.hasNext()){
			...
			DataUploadDetailsBeanFactory lala = new DataUploadDetailsBeanFactory(organizationId, startDate, endDate);
	...
	public List<DataUploadDetailsBean> getUploads(JSONObject json) {
		...
		while (it.hasNext()){
			...
			populateFactories(datasetId);
			transformationsderivedCollection = derivedBeanFactory.getTransformations();
	
					
			
			
		DataUploadDetailsBean dataUploadDetailsBean = new DataUploadDetailsBean(name, created, lastModified, creatorId, creatorName, itemCount,validItems,invalidItems, itemizerStatus, organizationId, organizationName,transformationsderivedCollection);
	...
	protected void populateFactories(String datasetId){
		 derivedBeanFactory = new DerivedTransformationDetailsBeanFactory(organizationId, datasetId, startDate, endDate);
	}
