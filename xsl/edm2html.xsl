...
<head>
 ...
 <![endif]--> <!--[if IE 9]><link rel="stylesheet" href="http://www.europeana.eu/portal/themes/default/css/min/ie9.min.css"/><![endif]--> <style>
...
</style> <!--[if IE]><link rel="stylesheet" href="http://www.europeana.eu/portal/themes/default/css/min/ie.min.css"/><![endif]--> 
<!--[if lte IE 7]><link rel="stylesheet" href="http://www.europeana.eu/portal/themes/default/css/min/ie7.min.css"/><![endif]--> <!--[if lte IE 8]>
<link rel="stylesheet" href="http://www.europeana.eu/portal/themes/default/css/min/ie8.min.css"/><![endif]--> <noscript>
<link rel="stylesheet" href="http://www.europeana.eu/portal/themes/default/css/min/noscript.min.css"/></noscript>
 <link rel="apple-touch-icon" href="http://www.europeana.eu/portal/themes/default/images/apple-touch-icon.png"/>
 
 
</head> <body class="locale-en"> 
  ...
  <xsl:for-each select="/rdf:RDF/edm:ProvidedCHO[@rdf:about=$thisabout]">
   ...
   <xsl:variable name="description">  
     <xsl:if test="current()/dc:description[1]">    
             <xsl:value-of select="current()/dc:description[1]"></xsl:value-of>
     </xsl:if>  
   </xsl:variable>
 ...
 <xsl:variable name="itemtype">  
     <xsl:if test="current()/edm:type">    
             <xsl:value-of select="current()/edm:type/text()"></xsl:value-of>
     </xsl:if>  
   </xsl:variable>
  
  <xsl:variable name="europeanaim">
	<xsl:choose>
	<xsl:when test="contains(lower-case($itemtype),'text')"><xsl:value-of select="'http://europeanastatic.eu/api/image?type=TEXT'"/></xsl:when>
    <xsl:when test="contains(lower-case($itemtype),'video')"><xsl:value-of select="'http://europeanastatic.eu/api/image?type=VIDEO'"/></xsl:when>
	<xsl:when test="contains(lower-case($itemtype),'image')"><xsl:value-of select="'http://europeanastatic.eu/api/image?type=IMAGE'"/></xsl:when>
	<xsl:when test="contains(lower-case($itemtype),'sound')"><xsl:value-of select="'http://europeanastatic.eu/api/image?type=SOUND'"/></xsl:when>
	<xsl:when test="contains(lower-case($itemtype),'3d')"><xsl:value-of select="'http://europeanastatic.eu/api/image?type=3D'"/></xsl:when>
	</xsl:choose>
</xsl:variable>
   ...
   <xsl:variable name="publisher">
    ...
    <xsl:for-each select="/rdf:RDF/ore:Aggregation/edm:aggregatedCHO[@rdf:resource=$thisabout]">
		    ...
		    <xsl:variable name="rightstitle">
				...
				<xsl:value-of select="' Public Domain Mark - No Known Copyright'"/></xsl:when>
				<xsl:when test="contains($rights, 'publicdomain/zero/1.0')">
				<xsl:value-of select="'CC0 - No Rights Reserved'"/></xsl:when>
				<xsl:when test="contains($rights, '/by/')">
				<xsl:value-of select="'Attribution'"/></xsl:when>
				<xsl:when test="contains($rights, '/by-sa/')">
				<xsl:value-of select="'Attribution-ShareAlike'"/></xsl:when>
				<xsl:when test="contains($rights, '/by-nc/')">
				<xsl:value-of select="'Attribution, Non-Commercial'"/></xsl:when>
				<xsl:when test="contains($rights, '/by-nd/')">
				<xsl:value-of select="'Attribution, No Derivatives'"/></xsl:when>
				<xsl:when test="contains($rights, '/by-nc-nd/')">
				<xsl:value-of select="'Attribution, Non-Commercial, No Derivatives'"/></xsl:when>
				<xsl:when test="contains($rights, '/by-nc-sa/')">
				<xsl:value-of select="'Attribution, Non-Commercial, ShareAlike'"/></xsl:when>
				</xsl:choose>
				</xsl:if>
		    ...
		    <xsl:variable name="rightsspan">
				...
				<xsl:choose>
				<xsl:when test="contains($rights, '/rr-f')">
				<span class="rights-icon icon-copyright" title="Rights Reserved - Free access"></span></xsl:when>
				<xsl:when test="contains($rights, '/rr-p')">
				<span class="rights-icon icon-copyright" title="Rights Reserved - Paid access"></span>
				</xsl:when>
				<xsl:when test="contains($rights, '/rr-r')">
				<span class="rights-icon icon-copyright" title="Rights Reserved - Restricted access"></span>
				</xsl:when>
				<xsl:when test="contains($rights, '/unknown')">
				<span class="rights-icon icon-unknown" title="Unknown"></span>
				</xsl:when>
				</xsl:choose>
				</xsl:if> 
				<xsl:if test="contains($rights, 'creativecommons.org')">
				<xsl:choose>
				<xsl:when test="contains($rights, 'publicdomain/mark/1.0/')">
				<span class="rights-icon icon-pd" title="Public Domain marked"></span>
				</xsl:when>
				<xsl:when test="contains($rights, 'publicdomain/zero/1.0')">
				<span class="rights-icon icon-cczero" title="CC0"></span>
				</xsl:when>
				<xsl:when test="contains($rights, '/by/')">
				<span class="rights-icon icon-cc" title="CC BY"></span> <span class="rights-icon icon-by" title="CC BY"></span> 
				</xsl:when>
				<xsl:when test="contains($rights, '/by-sa/')">
				  <span class="rights-icon icon-cc" title="CC BY-SA"></span> <span class="rights-icon icon-by" title="CC BY-SA"></span> <span class="rights-icon icon-sa" title="CC BY-SA"></span>
				</xsl:when>
				<xsl:when test="contains($rights, '/by-nc/')">
				  <span class="rights-icon icon-cc"/><span class="rights-icon icon-by"/><span class="rights-icon icon-sa"/>
				</xsl:when>
				<xsl:when test="contains($rights, '/by-nd/')">
				<span class="rights-icon icon-cc" title="CC BY-ND"></span> <span class="rights-icon icon-by" title="CC BY-ND"></span> <span class="rights-icon icon-nd" title="CC BY-ND"></span>
				</xsl:when>
				<xsl:when test="contains($rights, '/by-nc-nd/')">
				 <span class="rights-icon icon-cc" title="CC BY-NC-ND"></span> <span class="rights-icon icon-by" title="CC BY-NC-ND"></span> <span class="rights-icon icon-nceu" title="CC BY-NC-ND"></span> <span class="rights-icon icon-nd" title="CC BY-NC-ND"></span>
				</xsl:when>
				<xsl:when test="contains($rights, '/by-nc-sa/')">
				<span class="rights-icon icon-cc" title="CC BY-NC-SA"></span> <span class="rights-icon icon-by" title="CC BY-NC-SA"></span> <span class="rights-icon icon-nceu" title="CC BY-NC-SA"></span> <span class="rights-icon icon-sa" title="CC BY-NC-SA"></span>
				</xsl:when>
				</xsl:choose>
				</xsl:if>
		    ...
		    <xsl:if test="../edm:isShownAt/@rdf:resource"><xsl:value-of select="../edm:isShownAt/@rdf:resource"/></xsl:if>
		    </xsl:variable>
		    
		    <xsl:variable name="viewprovider">
		    <xsl:choose><xsl:when test="../edm:dataProvider/text()"><xsl:value-of select="../edm:dataProvider/text()"/></xsl:when><xsl:otherwise><xsl:value-of select="../edm:provider/text()"/></xsl:otherwise></xsl:choose>
		     </xsl:variable>
		    
			
			<div class="containereuropeana"><div id="ajax-feedback"></div> 
			<div id="content" class="row"> <div class="twelve columns"> 
			<div class="row">
		    ...
		    <div class="nine-cols-fulldoc" id="main-fulldoc-area"> 
			 <div id="excerpt">  
					<div id="item-details"> 
						 <div class="sidebar-right hide-on-x"> <div> 
						 <strong>Search also for:</strong> </div> 
						 <xsl:if test="string-length($title)&gt;0">
						 <strong>Title</strong> 
						 <ul> <li>
						 <xsl:variable name="titlehref">http://www.europeana.eu/portal/search.html?query=title%3a%22<xsl:value-of select="$title"></xsl:value-of>%22&amp;rows=24</xsl:variable>
						 
						     <a><xsl:attribute name="href"><xsl:copy-of select="$titlehref"/></xsl:attribute>
						...
						<xsl:if test="../../edm:ProvidedCHO[@rdf:about=$thisabout]/dc:creator/text() or ../../edm:ProvidedCHO[@rdf:about=$thisabout]/dc:contributor/text()">	 
						  <strong>Who</strong> <ul> 
						 <xsl:for-each select="../../edm:ProvidedCHO[@rdf:about=$thisabout]/dc:creator/text()">
						   <li><a><xsl:attribute name="href">http://www.europeana.eu/portal/search.html?query=who%3a%22<xsl:value-of select="."/>%22&amp;rows=24</xsl:attribute>
				      	    <xsl:attribute name="class">europeana</xsl:attribute>
						...
						<xsl:if test="../../edm:ProvidedCHO[@rdf:about=$thisabout]/dc:subject/text() or ../../edm:ProvidedCHO[@rdf:about=$thisabout]/dc:format/text() or ../../edm:ProvidedCHO[@rdf:about=$thisabout]/dc:type/text()">	 
						  <strong>What</strong> <ul> 
						 <xsl:for-each select="../../edm:ProvidedCHO[@rdf:about=$thisabout]/dc:type/text()">
						   <li><a><xsl:attribute name="href">http://www.europeana.eu/portal/search.html?query=what%3a%22<xsl:value-of select="."/>%22&amp;rows=24</xsl:attribute>
				      	    <xsl:attribute name="class">europeana</xsl:attribute>
							 <xsl:value-of select="."/></a>
							 </li>
						 </xsl:for-each>
					     ...
					     <xsl:if test="../edm:dataProvider or ./edm:provider">	 
						  <strong>Provider</strong> <ul> 
						 <xsl:for-each select="../edm:dataProvider/text()">
						   <li><a><xsl:attribute name="href">http://www.europeana.eu/portal/search.html?query=DATA_PROVIDER%3a%22<xsl:value-of select="."/>%22&amp;rows=24</xsl:attribute>
				      	    <xsl:attribute name="class">europeana</xsl:attribute>
							 <xsl:value-of select="."/></a>
							 </li>
						 </xsl:for-each>
						 <xsl:for-each select="../edm:provider/text()">
						   <li><a><xsl:attribute name="href">http://www.europeana.eu/portal/search.html?query=PROVIDER%3a%22<xsl:value-of select="."/>%22&amp;rows=24</xsl:attribute>
				      	    <xsl:attribute name="class">europeana</xsl:attribute>
							 <xsl:value-of select="."/></a>
							 </li>
						 </xsl:for-each>
			 ...
			 <h1 class="hide-on-phones" property="http://purl.org/dc/elements/1.1/title name dc:title"><xsl:copy-of select="$title"/></h1> 
			    <xsl:if test="string-length($alternative)&gt;0">
			     <div class="item-metadata"><span class="bold">Alternative Title: </span><xsl:copy-of select="$alternative"/></div>
			    </xsl:if>    
			 
			 
			  <xsl:if test="string-length($description)&gt;0">
			 <div id="item-description" class="item-metadata"> <span class="bold">Description:</span> 
			 ...
			 <xsl:copy-of select="$subject"/></div>
			 </xsl:if>
			  <xsl:if test="string-length($creator)&gt;0">
			  <div class="item-metadata"><span class="bold">  Creator: </span><xsl:copy-of select="$creator"/></div>
			    </xsl:if>    
			  
			  <xsl:if test="string-length($contributor)&gt;0">
			  <div class="item-metadata"> <span class="bold"> Contributor: </span><xsl:copy-of select="$contributor"/></div>
			    </xsl:if>    
			  
			  
			    
			     <xsl:if test="string-length($coverage)&gt;0">
			    <div class="item-metadata"> <span class="bold">Place/Period: </span> <xsl:copy-of select="$coverage"/></div>
			    </xsl:if>  
			  
			     <xsl:if test="string-length($place)&gt;0">
			    <div class="item-metadata"> <span class="bold">Place:  </span><xsl:copy-of select="$place"/></div>
			    </xsl:if>  
			  
			  <xsl:if test="string-length($date)&gt;0">
			    <div class="item-metadata"> <span class="bold">Date:  </span><xsl:copy-of select="$date"/></div>
			    </xsl:if>  
			   
			    <xsl:if test="string-length($temporal)&gt;0">
    			<div class="item-metadata"><span class="bold">Time Period:</span>  <xsl:copy-of select="$temporal"/></div>
    			</xsl:if>     
 
  
			   <xsl:if test="string-length($issued)&gt;0">
			    <div class="item-metadata"><span class="bold">Publication date: </span> <xsl:copy-of select="$issued"/></div>
			    </xsl:if>     

  
			   <xsl:if test="string-length($type)&gt;0">
			    <div class="item-metadata"><span class="bold">Type:  </span><xsl:copy-of select="$type"/></div>
			    </xsl:if>     
  
  				<xsl:if test="string-length($format)&gt;0">
			    <div class="item-metadata"><span class="bold">Format:  </span><xsl:copy-of select="$format"/></div>
			    </xsl:if>     
			  
    
			    <xsl:if test="string-length($identifier)&gt;0">
			    			    <div class="item-metadata"><span class="bold">Identifier:</span>  <xsl:copy-of select="$identifier"/></div>
			    </xsl:if>    
			    
				<xsl:if test="string-length($relation)&gt;0">
			     <div class="item-metadata"><span class="bold">Relation: </span><xsl:copy-of select="$relation"/></div>
			   </xsl:if> 
			    
     		   <xsl:if test="string-length($partof)&gt;0">
			     <div class="item-metadata"><span class="bold">Is part of: </span><xsl:copy-of select="$partof"/></div>
			   </xsl:if>
   
			   <xsl:if test="string-length($haspart)&gt;0">
			   <div class="item-metadata"><span class="bold">Has part:</span> <xsl:copy-of select="$haspart"/></div>
			   </xsl:if>
			    
			   <xsl:if test="string-length($nsequence)&gt;0">
			   <div class="item-metadata"><span class="bold">Next in sequence: </span><xsl:copy-of select="$nsequence"/></div>
			   </xsl:if>
     
			   <xsl:if test="string-length($language)&gt;0">
			    <div class="item-metadata"><span class="bold">Language: </span><xsl:copy-of select="$language"/></div>
			    </xsl:if>
		
			   <xsl:if test="string-length($dcrights)&gt;0">
			    <div class="item-metadata"><span class="bold">Rights: </span><xsl:copy-of select="$dcrights"/></div>
			    </xsl:if>
	
			   <xsl:if test="string-length($provenance)&gt;0">
			    <div class="item-metadata"><span class="bold">Provenance: </span><xsl:copy-of select="$provenance"/></div>
			    </xsl:if>
   
			   <xsl:if test="string-length($publisher)&gt;0">
			    			    <div class="item-metadata"><span class="bold">Publisher: </span><xsl:copy-of select="$publisher"/></div>
			    </xsl:if> 
   
				   <xsl:if test="string-length($source)&gt;0">
				    <div class="item-metadata"><span class="bold">Source: </span><xsl:copy-of select="$source"/></div>
				    </xsl:if>
   
			  <xsl:if test="string-length($dataprovider)&gt;0">
    			<div class="item-metadata"><span class="bold">Data provider: </span><xsl:copy-of select="$dataprovider"/></div>
		    </xsl:if>
		    
		     
		    <xsl:if test="string-length($provider)&gt;0">
		    <div class="item-metadata"><span class="bold">Provider: </span><xsl:copy-of select="$provider"/></div>
		    </xsl:if>  
    ...
    </xsl:for-each><!-- ore:Aggregation -->
 
  </xsl:for-each>  <!-- providedCHO -->  
...
<xsl:choose>
<xsl:when test="$mynode/dc:title"> <xsl:for-each select="$mynode/dc:title">
    		<xsl:value-of select="concat(.,'&#xA;')"></xsl:value-of>
			</xsl:for-each></xsl:when>
<xsl:otherwise>
        <xsl:choose>
		<xsl:when test="$mynode/dc:description[@xml:lang='en']"><xsl:value-of select="dc:description[@xml:lang='en']"></xsl:value-of></xsl:when>
		<xsl:otherwise>
		 <xsl:if test="$mynode/dc:description[1]">	
		   ...
		   <xsl:if test="string-length( $mynode/dc:description[1] ) &lt; 49">	
		     <xsl:value-of select="$mynode/dc:description[1]"></xsl:value-of>
		   </xsl:if>
		 </xsl:if>  
		</xsl:otherwise>
		</xsl:choose>
...
<xsl:param name="europeanaim"/>
	...
	<div class="three-cols-fulldoc-sidebar"> 
					...
					<h2 id="phone-object-title" class="show-on-phones" aria-hidden="true"><xsl:value-of select="$title"/></h2> 
					<div style="position: relative; text-align: center" id="carousel-1-img-measure"> 
					 <img>
				        <xsl:attribute name="src">
				       <xsl:choose>
						  <xsl:when test="string-length($imagesrc)&gt; 0"><xsl:value-of select="$imagesrc"/></xsl:when>
						  <xsl:otherwise><xsl:value-of select="$europeanaim"/></xsl:otherwise></xsl:choose></xsl:attribute>
				       <xsl:attribute name="style">cursor: pointer;max-width: 200px; height: auto;</xsl:attribute>
				       <xsl:attribute name="class"></xsl:attribute> 
				     </img>
		           
					 
					<div style="display: none;" class="lb-trigger"><span rel="#lightbox" title="View" class="icon-magplus label">View</span></div>
					</div> <div id="carousel-1" class="europeana-bordered"> 
					</div> 
					<div class="original-context"> 
					
					<a><xsl:attribute name="href"><xsl:value-of select="$rights"/></xsl:attribute>
					    <xsl:attribute name="target">_blank</xsl:attribute>
			      	    <xsl:attribute name="class">item-metadata  rights-badge</xsl:attribute>
			      	    <xsl:copy-of select="$rightsspan"/>					
					<span class="rights-text"><xsl:value-of select="$rightstitle"/></span></a> 
					<div class="clear">View item at</div> 
					<a><xsl:attribute name="href"><xsl:copy-of select="$viewat"/></xsl:attribute>
					...
					<div class="actions"> <div class="action-link shares-link">
					 <span class="icon-share" title="Share item on Facebook, Twitter, etc.">
					 <span class="action-title" title="Share item on Facebook, Twitter, etc.">Share</span></span> </div> 
					 <div> <a href="" id="citation-link" class="icon-cite action-link" title="Click to cite this object" rel="nofollow"> 
					 <span class="action-title">Cite on Wikipedia</span> </a> </div> <div id="translate-container"> <span class="icon-translate"></span>
					 <a class="disabled" href="" id="translate-item"> Translate details<span class=""></span> </a>
					 </div> </div> </div> 
			 </div> 
</xsl:template>
...
<xsl:if test="$mynode/dcterms:alternative"> <xsl:for-each select="$mynode/dcterms:alternative">
    		<xsl:value-of select="concat(.,'&#xA;')"></xsl:value-of>
			</xsl:for-each></xsl:if>
    ...
    <xsl:for-each select="$mynode">
         <xsl:variable name="textvar">
           ...
           <xsl:when test="string-length($link)&gt;0">
               <a><xsl:attribute name="href"><xsl:value-of select="$link" /></xsl:attribute><xsl:value-of select="$textlink" /></a><xsl:value-of select="$separator"/>
           	</xsl:when>
    		<xsl:otherwise>
    		     <xsl:value-of select="$textvar"></xsl:value-of><xsl:value-of select="$separator"/>
    		</xsl:otherwise>
    		</xsl:choose>
